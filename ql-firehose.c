...
#define error_return()  do {printf("%s %s %d fail\n", __FILE__, __func__, __LINE__); return __LINE__; } while(0)

const char *q_device_type = "nand";
...
static int fh_fixup_program_cmd(struct fh_data *fh_data, struct fh_cmd *fh_cmd, long* filesize_out) {
    ...
    *filesize_out = filesize;
    ...
    if (filesize <= 0) {
        printf("fail to ftell %s, errno: %d (%s)\n", full_path, errno, strerror(errno));
        fh_cmd->program.num_partition_sectors = 0;
...
static int _fh_recv_cmd(struct fh_data *fh_data, struct fh_cmd *fh_cmd, unsigned timeout) {
    ...
    ret = qdl_read(fh_data->usb_handle, fh_data->xml_rx_buf, fh_data->xml_rx_size, timeout);
    if (ret <= 0) {
        return -1;
    }
...
static int fh_send_cmd(struct fh_data *fh_data, const struct fh_cmd *fh_cmd) {
    ...
    tx_len = qdl_write(fh_data->usb_handle, xml_buf, strlen(xml_buf));
        ...
        while (1)
        {
            ...
            while (chunk_data_sz >= fh_data->MaxPayloadSizeToTargetInBytes)
            {
                ...
                if (reads > 0) {
                    ...
                    size_t writes = qdl_write(fh_data->usb_handle, pbuf, reads);
                    if (reads != writes) {
                    ...
                    //printf("filesend=%zd, filesize=%zd\n", filesend, filesize);
                } else {
                    break;
                }
            ...
            {
                printf("%s filesend=%zd, filesize=%zd\n", __func__, filesend, filesize);
                break;
            }
        }
    }
...
static int fh_process_patch(struct fh_data *fh_data, const struct fh_cmd *fh_cmd)
{
    ...
    if (fh_wait_response_cmd(fh_data, &fh_rx_cmd, timeout) != 0) //SDX55 need 4 seconds
    {
        printf("fh_process_patch : fh_wait_response_cmd fail\n");
        error_return();
    }
    if (strcmp(fh_rx_cmd.response.value, "ACK"))
    {
        printf("fh_process_patch : response should be ACK\n");
        error_return();
    }
...
int firehose_main(char *firehose_dir, void *usb_handle, unsigned qusb_zlp_mode) {
    unsigned x;
    ...
    {
        {
            if (!qfile_find_file(firehose_dir, "rawprogram_", ".xml", &xmlfile_list[xmlfile_cnt])
                ...
                printf("retrieve rawprogram namd file failed.\n");
                //error_return();
            }
            else
                xmlfile_cnt++;
        ...
        if (!qfile_find_file(firehose_dir, "patch_", ".xml", &xmlfile_list[xmlfile_cnt])
            ...
            printf("retrieve patch namd file failed.\n");
            //error_return();
        }
        else
            xmlfile_cnt++;
...
int firehose_flash(char *firehose_dir)
{
    struct qdl_device qdl;
    char firehose_mbn[256] = {};

    snprintf(firehose_mbn, sizeof(firehose_mbn), "%s/prog_nand_firehose_9x55.mbn", firehose_dir);

    if (access(firehose_mbn, F_OK) != 0)
    {
	...
	firehose_sahara(&qdl, firehose_mbn);
	firehose_main(firehose_dir, &qdl, 1);
